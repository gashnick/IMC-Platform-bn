...
const compareUsers = (expected: User, actual: User) => {
    ...
    expect(new Date(actual.createdAt || "").toISOString()).toEqual(
...
describe("User API Endpoints", () => {
    ...
    beforeAll(async () => {
        ...
        auth_token = loginResponse.headers["set-cookie"].find((cookie: string) =>
    ...
    describe("GET /users", () => {
        ...
        const response = await request(app)
            .get("/users")
            .set("Cookie", auth_token); // Include the httpOnly cookie
